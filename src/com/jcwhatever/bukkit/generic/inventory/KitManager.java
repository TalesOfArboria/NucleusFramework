...
public class KitManager {
     ...
     * @param settings  Config section settings to store and retrieve kit information.
     */
    public KitManager(Plugin plugin, IDataNode settings) {
        _plugin = plugin;
        _settings = settings;
        _kitList = new LinkedList<Kit>();

        Set<String> kits = settings.getSubNodeNames();
        _kits = new HashMap<String, Kit>(kits.size() + 10);

        for (String kitName : kits) {
            Kit kit = new Kit(plugin, kitName);
            
            ItemStack[] items = settings.getItemStacks(kitName + ".items");
            ItemStack[] armor = settings.getItemStacks(kitName + ".armor");
