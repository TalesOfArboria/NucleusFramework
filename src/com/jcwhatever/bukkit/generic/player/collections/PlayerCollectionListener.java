...
final class PlayerCollectionListener implements Listener {
        ...
        if (_listener == null) {
            synchronized (_sync) {
                if (_listener == null) {
                    ...
                    PluginManager pm = GenericsLib.getInstance().getServer().getPluginManager();
                    pm.registerEvents(_listener, GenericsLib.getInstance());
                }
            }
        }
	...
	private void removePlayer(Player p) {
        ...
        Scheduler.runTaskLaterAsync(GenericsLib.getInstance(), 1, new RemovePlayer(p, collections));
	}
